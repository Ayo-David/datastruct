//Comments on MAX_VALUE, Double Conditions, sorting with (a,b)=>a-b
  ...
  for (let size = 1; size <= len; size++) {
    ...
    if (len % size == 0) {
      ...
      for (let peak of peaks) {
        if (Math.floor(peak / size) > find) break;
...
function flags(A) {
  ...
  let max_flag = Math.ceil(Math.sqrt(peaks[peaks.length - 1] - peaks[0])),
    max = 1;
...
function maxSliceSum(A) {
  ...
  for (let i = 1; i <= A.length - 1; i++) {
    ...
    console.log(
      "and the highest sum at this point is: ",
      highest_sum,
      " and the highest is now",
      highest,
      "\n========"
    );
  }
...
function maxSliceSum2(A) {
  ...
  for (let i = 1; i < A.length; i++) {
    let sum = slice + A[i];
    slice = Math.max(sum, A[i]); //at this point discard preceeding slice if the current value is higher than their sum
    highSum = Math.max(highSum, slice);
    console.log(`highSum = `, highSum);
  }
  return highSum;
}
console.log(maxSliceSum2(A));
...
function stonewall(H) {
  ...
  console.log(
    `counted building blocks ${countBlock}, the remaining stack blocks ${stack.length}`
  );
  ...
  for (s of S) {
    if (s == ")") {
      ...
      console.log(
        "remaining top is: ",
        top,
        ", one set of bracket has been remove"
      );
    } else {
...
let S = "([)()]";
//console.log(`stackBracket = `, StackBracket(S));
...
function max3product(A) {
  A.sort((a, b) => a - b); //A.sort() is incorrect for -ve numbers
...
//the idea is that after sorting the product of last
//3 or 1st 2 and last number will be the highest product
function maxThree(A) {
  // write your code in JavaScript (Node.js 8.9.4)
  A.sort((a, b) => a - b);
  let len = A.length;
...
function missint(A) {
  ...
  //using set, discard all no less than zero and then map it with the index of the
  ...
  //we'll start checking from 1 since we are not interested in 0
  for (let i = 1; i <= A.length + 1; i++) {
    if (!S.has(i)) return i;
  }
}

function missingInt3(A) {
  //my solution - most prefered
  //var onlyPositiveInt = [];
...
function permMissing(A) {
  ...
  //using object hash
  //use !hash[a] to check if the key (element) does not exist in the object
  function permMissing2(A) {
    // Implement your solution here
    let hash = {};
    for (let a of A) {
      if (!hash[a]) {
...
function oddOccurrence2(A) {
  ...
  for (let i in hash) {
    if (hash[i] % 2 === 1) return parseInt(i);
  }
}
//let A = [9, 3, 9, 3, 9, 7, 9]
...
function frogJ(X, Y, D) {
  let L = Y - X;
  let count = L / D;
  return Math.ceil(count);
}

//console.log(`frogJ = `, frogJ(10,85,30))
...
//console.log(`PermMiss = `, PermMiss([2,3,1,5]))
...
//[1,3,1,4,5,3,2,4]
//console.log(`frogR = `, frogR([1, 3, 1, 4, 2, 3, 5, 4], 5));

function PermCh(A) {}

//console.log(`PermCh = `, PermCh);
